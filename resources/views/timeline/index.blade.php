    ...
    <div class="col-lg-6">
        <form role="form" action="{{ route('status.post') }}" method="post">
            <div class="form-group{{ $errors->has('status') ? ' has-error' : '' }}">
                <textarea placeholder="hello {{ Auth::user()->getFirstNameOrUsername() }}" name="status" class="form-control" rows="2"></textarea>
...
<div class="row">
    <div class="col-lg-5">
    <!--display the user's status if any-->
        @if (!$statuses->count())
    <p>There is nothing in your status</p>
        @else
